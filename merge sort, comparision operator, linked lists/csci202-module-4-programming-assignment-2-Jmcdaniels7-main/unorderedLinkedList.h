...
public:
    ...
    void mergeSort(int (*comp)( type &, type &));
...
template <class type>
void unorderedLinkedList<type>::insert(const type &newdata)
{
    node<type> *newNode;
...
template <class type>
void unorderedLinkedList<type>::mergeSort(int (*comp)(type &item1, type &item2))
{
    this->compare = comp;
...
node<type> *unorderedLinkedList<type>::mergeList(node<type> *first1, node<type> *first2)
{
    ...
    else
    {
        ...
        if (compare(*first1->data, *first2->data) == -1)
        {
            newHead = first1;
            first1 = first1->link;
            lastSmall = newHead;
